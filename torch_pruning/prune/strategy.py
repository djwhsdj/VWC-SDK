...
class LNStrategy(BaseStrategy):
    ...
    # -> return값이 어떠한 상태인지를 명시해주기 위해 사용한다.
    def apply(self, weights, to_p_amount=0.0, round_to=1)->  Sequence[int]:  # return index
        if to_p_amount<=0: return []
        n = len(weights)
        l1_norm = torch.norm( weights.view(n, -1), p=self.p, dim=1 )
        n_to_prune = to_p_amount
        n_to_prune = round_pruning_amount(n, n_to_prune, round_to)
        if n_to_prune == 0: return []
