...
def train_model(model, train_loader, test_loader):
    ...
    if args.scheduler == 'StepLR' :
        ...
        # scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[75, 125, 175], gamma=0.1) # 스케쥴러 설정 [150, 250, 350]
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[30, 50, 70], gamma=0.1) # 스케쥴러 설정 [150, 250, 350]
        # 스케쥴러 값 수정해볼 것.
    else :
    ...
    for epoch in range(args.total_epochs):
        ...
        if best_acc<acc:
            if args.q == 0 :
                ...
                torch.save( model, args.directory +'/ckpt_cifar10/512x256_resnet20_ab_%d_wb_%d_e_%d_round%d.pth'%(args.ab, args.wb, args.total_epochs, args.round) )
                # if epoch <= 120 :
...
def prune_model(model):
    ...
    bit_precision = 2
    array[1] = int(array[1]/bit_precision)
    block_pruned = []
    block_pruned1 = []
    tiled_ic = []
    after_ic = []
    for i in range(len(images)) :
        t_IC = vw_sdk(images[i], images[i], 3, 3, IC[i], IC[i], array[0], array[1], 1)
        res_block = IC[i] % t_IC
